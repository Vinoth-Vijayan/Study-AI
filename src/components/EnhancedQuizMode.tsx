...
import { ArrowLeft, CheckCircle, AlertTriangle, Brain, Download, Trophy, Target } from "lucide-react";
...
const EnhancedQuizMode = ({
  ...
  const startQuiz = async () => {
    ...
    try {
      const result = await generateAdvancedQuestions(
        file,
        pageRange,
        difficulty,
        questionsPerPage,
  ...
  const handleDownloadQuestions = async () => {
    try {
      await downloadPDF({
        title: `TNPSC Quiz - ${difficulty.toUpperCase()} - Pages ${pageRange.start}-${pageRange.end}`,
        content: questions,
        type: 'questions'
      });
  ...
  if (!quizStarted) {
    return (
      <Card className="p-8 text-center bg-white/80 backdrop-blur-sm shadow-lg border-0">
        <div className="flex items-center gap-3 mb-6 justify-center">
          <Brain className="h-8 w-8 text-blue-600" />
          <h2 className="text-2xl font-bold text-gray-800">TNPSC Enhanced Quiz</h2>
        </div>

        <div className="bg-gradient-to-r from-blue-50 to-purple-50 p-6 rounded-lg mb-6">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-center">
            <div>
              <div className="text-2xl font-bold text-blue-600">{pageRange.end - pageRange.start + 1}</div>
              <div className="text-sm text-gray-600">Pages</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-green-600">{(pageRange.end - pageRange.start + 1) * questionsPerPage}</div>
              <div className="text-sm text-gray-600">Questions</div>
            </div>
            <div>
              <div className="text-2xl font-bold text-orange-600">{questionsPerPage}</div>
        ...
        <p className="text-gray-600 mb-6">
          Ready to test your knowledge on pages {pageRange.start} to {pageRange.end}?<br/>
        ...
        <div className="flex gap-4 justify-center">
          ...
          <Button onClick={onBackToAnalyzer} variant="outline">
    ...
    return (
      <div className="space-y-6">
        ...
        <Card className="p-6 bg-white/80 backdrop-blur-sm shadow-lg border-0">
          ...
          <div className="flex gap-4 justify-center">
            <Button onClick={handleDownloadQuestions} variant="outline">
              ...
              <div className="flex items-start justify-between mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  <Badge className="bg-purple-100 text-purple-700">
                    Page {answer.question.pageNumber}
                  </Badge>
                </div>
              </div>
      ...
      <Card className="p-6 bg-white/80 backdrop-blur-sm shadow-lg border-0">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <h2 className="text-xl font-bold text-gray-800">TNPSC Enhanced Quiz</h2>
          </div>
          <div className="flex gap-2">
            <Badge className="bg-blue-100 text-blue-700">
